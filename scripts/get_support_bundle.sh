...
for pod in $(kubectl ${KUBE_OPTS} get pods -l role=elasticsearch -n ${NAMESPACE} | grep -v "NAME" | awk '{print $1}')
do
    printf "$pod\t" |tee -a elasticsearch_storage.log
    kubectl ${KUBE_OPTS} exec -it $pod -n ${NAMESPACE} -- df -Ph | grep elasticsearch | grep -v "tmpfs" | awk '{printf "%-13s %10s %6s %8s %6s %s\n",$1,$2,$3,$4,$5,$6}' |tee -a elasticsearch_storage.log
done;
...
do
    echo $pod; for cmd in proxyhistograms status tpstats compactionstats; do kubectl exec $pod -c cassandra -n sysdig -- nodetool $cmd > ${LOG_DIR}/cassandra_nodes_output.log;
done;
done;
